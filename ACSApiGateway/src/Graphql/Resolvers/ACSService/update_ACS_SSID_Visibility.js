...
exports.update_ACS_SSID_Visibility = async (error, args, CONTEXT) => {
  console.log("ayaya");
  try {
      ...
      if (Body.messagecode === 0) {
        ...
        if (AcsUrlDT && AcsUrlDT.URL) {
          let variable = {
            ...
            parameterValues: [
              [
                `InternetGatewayDevice.LANDevice.1.WLANConfiguration.${sExistingPositionSSId}.SSIDAdvertisementEnabled`,
                args.bSSIDVisibility,
                "xsd:string",
              ],
            ],
          };
